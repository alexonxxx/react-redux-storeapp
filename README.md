...
- XXX
  ...
  There are some XXX across the src/ of the project,
  you should find all of them and replace by the 
  correct logic.

- Catalog 

  [ ] Connect the ProductList to the State
  [ ] ProductListItem should change button text 
      if there are is any of that product bought

- Header

  [ ] Create a component to show the overview
      of the cart (XXX items (XXX €))
  [ ] Connect that component to the State 
      to show cart evolution in real time

- Product Detail

  [ ] Create ProductDetail component
  [ ] Connect ProductDetail to the state
  [ ] Add ProdutDetail connector to the View component
  [ ] Add picture, name, and price to the ProductDetail
  [ ] Add product quantity, all items price to ProductDetail
      (you might need to update the connector)
  [ ] Create and connect a button to increase one item
  [ ] Create and connect a button to decrease one item
      Make sure that decrease is disabled or hidden
      when there is less than 2 items in the cart
  [ ] Add increase and decrease buttons to ProductDetail

- Cart

  [ ] Create and connect a LineItemList 
  [ ] Add LineItemList connector to the View component
  [ ] Create a Drop button
  [ ] For each line item:
      [ ] show name, quantity, price, accumulated price
      [ ] add increase, decrease and drop buttons
